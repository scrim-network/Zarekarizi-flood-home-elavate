...
require(lhs)
pdf("Figure/value_uncertainty.pdf", width =2*3.94, height =3*2.43)
#par(mai=c(.4,.4,.4,.4))
par(cex=0.5,fig=c(0,0.5,1-0.33,1))
nsow=1000
z<- randomLHS(nsow, 1)
...
par(fig=c(0.5,1,1-0.33,1),new=T)
# Establish the damage-depth relationship 
EU_Depth <-           c(0, 1.64, 3.28, 4.92, 6.56, 9.84, 13.12, 16.40)
...
par(fig=c(0.25,0.75,0.3,.63),new=T)
...
polygon(x=c(0.4,0.6,0.6,0.4),y=c(0.4,0.4,0.45,0.45)+0.25,border=NA,col="gray")
arrows(x0=0.455,y0=0.79,x1=0.49,y1=.7,length=0.05,col="blue",lwd=2)
arrows(x0=0.5+0.045,y0=0.79,x1=0.51,y1=.7,length=0.05,col="blue",lwd=2)
text(0.5,0.425+0.25,'Multiply',col="blue",cex=2)
arrows(x0=0.5,y0=0.65,x1=0.5,y1=.585,length=0.05,col="blue",lwd=2)
#arrows(x0=0.5,y0=0.25,x1=0.5,y1=.15,length=0.05,col="blue",lwd=2)
text(-0.03,1.06,'a)',bty="n",xpd=T,cex=2)
text(0.55,1.06,'b)',bty="n",xpd=T,cex=2)
text(0.25,0.63,'c)',bty="n",xpd=T,cex=2)
