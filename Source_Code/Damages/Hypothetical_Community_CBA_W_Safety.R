...
rm(list=ls())
plot_hypothetical_community_safety_histogram=FALSE
plot_chain_of_hypothetical_houses_CBA.RData=FALSE
...
if(plot_chain_of_hypothetical_houses_CBA.RData==TRUE){
load("RData/chain_of_hypothetical_houses_CBA.RData")
FEMA_safety_seq <- opt_safety_seq <- rep(NA,n_sow)
 for(i in 1:n_sow){
   FEMA_safety_seq[i]=0.77
   opt_safety_seq[i]=lifetime_safety(life_span,mu,sigma,xi,35.4+SOWs[i,3],cba_opt_height[i])
 }
color_safety=rep(1,n_sow)
color_safety[opt_safety_seq>=0.78]=2
color_safety[opt_safety_seq<=0.76]=3
...
pdf("Figure/chain_of_hypothetical_houses_total_cost_comparison_with_FEMA_w_safety.pdf", width =3.94, height =2.43)
par(cex=0.5)
...
pdf("Figure/hypothetical_community_safety_histogram.pdf", width =3.94, height =2.43)
par(cex=0.5)
...
for (i in 1:n_sow){
  ...
  lines(1:3,c(punif(cba_opt_Totcost[i],min=min(cba_opt_Totcost),max=max(cba_opt_Totcost)),
              ...
              punif(opt_safety_seq[i],min=min(opt_safety_seq),max=max(opt_safety_seq))))
  }
}
}
